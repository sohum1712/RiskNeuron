...
// Search for the end of AnimatePresence and add the steps back
const insertionPoint = content.lastIndexOf('</AnimatePresence>');
if (insertionPoint !== -1) {
    const before = content.substring(0, insertionPoint);
    const after = content.substring(insertionPoint);
    
    // We need to check if step 4 is missing too
    if (!content.includes('step === 4')) {
        // Find Step 4 content from history... oh wait I have it in my memory.
        const step4Body = \`
                {assessmentLoading ? (
                   ...
                   <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}>
                    ...
                    <div className="grid md:grid-cols-3 gap-4">
                      {workerResult.recommended_plans.map(plan => (
                        <div key={plan.plan_type} className={\`border-2 rounded-xl p-4 \${plan.recommended ? 'border-[#F97316] bg-[#F97316]/5' : 'border-white/10'}\`}>
        ...
        content = before + step2 + step3 + step4Header + step4Body + after;
    } else {
        content = before + step2 + step3 + after;
    }
}

fs.writeFileSync(file, content);
console.log('Restored all missing Onboarding steps');
